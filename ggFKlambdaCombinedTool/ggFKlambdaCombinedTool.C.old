...
namespace ggFKlambdaCombinedTool
{
    using std::cerr;
    using std::clog;
    using std::cout;
...
void GenerateKLambaSamples(const double KLambda)
{
    ...
    {
        cerr << "error: "
             << "Can't open basefile, Check HERE_IS_BASE!" << endl;
        return;
    }
    ...
    {
        cerr << "error: "
             << "Can't open outputfile, Check HERE_IS_BASE!" << endl;
        return;
    }
    ...
    for (auto dirname_input = vec_dirnames.begin(); dirname_input != vec_dirnames.end(); dirname_input++)
    {
        ...
        {
            cerr << "error: "
                 << "Can't open TDirectory, Check dirName! " << *dirname_input << endl;
            return;
        }
    }
        ...
        case PNN:
        {
            ...
            for (auto iter_variable = ML_variable_names.begin(); iter_variable != ML_variable_names.end(); iter_variable++)
            {
                for (auto iter_region = region_names.begin(); iter_region != region_names.end(); iter_region++)
                {
                    // Reweighted histogram name
                    auto hist_name_reweighted = base_names.at("base") + string_KLambda + "fromReco" +
                                                "_" + *iter_region + "_" + *iter_variable + string_KLambda;
                    // Get three base histogram
                    auto h0_name = base_names.at("0p0") + "_" + *iter_region + "_" + *iter_variable + string_KLambda;
                    auto h1_name = base_names.at("1p0") + "_" + *iter_region + "_" + *iter_variable + string_KLambda;
                    auto h20_name = base_names.at("20p0") + "_" + *iter_region + "_" + *iter_variable + string_KLambda;
