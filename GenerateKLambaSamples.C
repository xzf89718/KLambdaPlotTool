...
{   
    ...
    for(auto iter_dir = vec_dirnames.begin(); iter_dir != vec_dirnames.end(); iter_dir++){
        output_file->cd(iter_dir.c_str());
        switch(iter_dir.c_str()){
            case "Preselection":
                ...
                break;
            case "BDTScorePreselection":
                break;
            case "PNNScorePreselection":
                break;
            case "NNScorePreselection":
                break;
                // Go back to ../
                gDirectory->cd("../");
        }
